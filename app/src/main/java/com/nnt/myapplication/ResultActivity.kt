...
class ResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(resultado < 16.0) {
            ...
            binding.saudavel.setTextColor(Color.parseColor("#2e7d32"))
            binding.intervaloSaudavel.setTextColor(Color.parseColor("#2e7d32"))
        } else if (resultado >= 25 && resultado < 30) {
            binding.imagemMedidor.setImageResource(R.drawable.sobrepeso)
            binding.sobrepeso.setTextColor(Color.parseColor("#FFA200"))
            binding.intervaloSobrepeso.setTextColor(Color.parseColor("#FFA200"))
        } else if (resultado >= 30 && resultado < 35) {
            binding.imagemMedidor.setImageResource(R.drawable.obesidade_grau_i)
            binding.obesidadeGrauI.setTextColor(Color.parseColor("#F2683C"))
            binding.intervaloObesidadeGrauIII.setTextColor(Color.parseColor("#F2683C"))
        } else if (resultado >= 35 && resultado < 40) {
            binding.imagemMedidor.setImageResource(R.drawable.obesidade_grau_ii)
            binding.obesidadeGrauII.setTextColor(Color.parseColor("#BE3A262"))
            binding.intervaloObesidadeGrauIII.setTextColor(Color.parseColor("#BE3A26"))
        } else {
            binding.imagemMedidor.setImageResource(R.drawable.obesidade_grau_iii)
            binding.obesidadeGrauIII.setTextColor(Color.parseColor("#861619"))
            binding.intervaloObesidadeGrauIII.setTextColor(Color.parseColor("#861619"))
        }
    }
}